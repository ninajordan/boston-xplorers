...
export function validateLocationCreation(req, res, next) {
  ...
  // Required fields
  if (!isNonEmptyString(locationName))
    errors.push("locationName is required and must be a non-empty string.");
  if (!isNonEmptyString(locationDescription))
    errors.push("locationDescription is required and must be a non-empty string.");
  ...
  if (locationImage !== undefined && locationImage !== null && locationImage !== "") {
    if (!isNonEmptyString(locationImage))
      errors.push("locationImage must be a non-empty string when provided.");
  }

  if (neighborhood !== undefined && neighborhood !== null && neighborhood !== "") {
    if (!isNonEmptyString(neighborhood))
      errors.push("neighborhood must be a non-empty string when provided.");
  }
...
export function validateLocationUpdate(req, res, next) {
  ...
  if ("locationImage" in body && body.locationImage !== null && body.locationImage !== "") {
    if (!isNonEmptyString(body.locationImage))
      errors.push("locationImage must be a non-empty string.");
  }

  if ("neighborhood" in body && body.neighborhood !== null && body.neighborhood !== "") {
    if (!isNonEmptyString(body.neighborhood))
      errors.push("neighborhood must be a non-empty string.");
  }
  ...
  if ("timeToComplete" in body) {
    if (!isValidTimeToComplete(body.timeToComplete)) {
      ...
      typeof body.timeToComplete === "string" &&
      !Number.isNaN(Number(body.timeToComplete))
    ) {
